...
const BlogList = () => {
    ...
    const BlogModal = ({ isOpen, onClose, blog }) => {
        ...
        const renderBodyContent = (item, index) => {
            switch (item.type) {
                case 'heading': {
                    ...
                    return (
                        <HeadingTag key={index} className="text-base sm:text-lg font-semibold text-[#333333] mt-3 sm:mt-4">
                            {item.content}
                        </HeadingTag>
                    );
                }
                case 'paragraph':
                    return (
                        <p key={index} className="text-[#718096] text-xs sm:text-sm mt-2">
                            {item.content}
                        </p>
                    );
                case 'image':
                    return (
                        <div key={index} className="mt-3 sm:mt-4">
                            <img
                                ...
                                className="w-full h-auto rounded-md max-w-[90vw] sm:max-w-md"
                                onError={(e) => {
                            ...
                            />
                            {item.caption && (
                                <p className="text-[#718096] text-xs mt-1">{item.caption}</p>
                            )}
                        </div>
                    );
                case 'list':
                    return (
                        <ul key={index} className={`mt-2 ${item.style === 'bullet' ? 'list-disc pl-5' : 'list-decimal pl-5'}`}>
                            {item.items.map((listItem, i) => (
                                <li key={i} className="text-[#718096] text-xs sm:text-sm">
                                    {listItem}
                                </li>
                            ))}
                        </ul>
        ...
        return (
            ...
            >
                <div
                    className="bg-white p-4 sm:p-6 rounded-lg w-full max-w-[90vw] sm:max-w-lg md:max-w-2xl max-h-[85vh] overflow-y-auto"
                    onClick={(e) => e.stopPropagation()}
                >
                    <div className="flex justify-between items-center mb-3 sm:mb-4">
                        <h2 className="text-lg sm:text-xl font-bold text-[#333333]">{blog.title}</h2>
                        <button
                            onClick={onClose}
                            className="text-[#718096] hover:text-[#9E0B7F] p-2 rounded-full transition duration-300"
                    ...
                    <div className="flex items-center text-xs sm:text-sm text-[#718096] mb-3 sm:mb-4">
                        <span>{blog.date}</span>
                    </div>
                    <div>
                        {blog.body.length > 0 ? (
                            ...
                            <p className="text-[#718096] text-xs sm:text-sm">
                                No content available for this blog.
                            </p>
                        )}
                        {!hasContent && blog.body.every((item) => item.type === 'image') && (
                            <p className="text-[#718096] text-xs sm:text-sm mt-2">
                                        ...
                                        <div className="group h-full relative py-2 px-2">
                                            ...
                                            >
                                                ...
                                                <div className="relative overflow-hidden h-48 md:h-56 lg:h-64">
                                                    ...
                                                    />
                                                    <div className="absolute right-4 bottom-4 bg-[#ADD01C] text-white px-3 pyversus2 rounded-lg text-center shadow-md transition-transform duration-300 group-hover:scale-105">
