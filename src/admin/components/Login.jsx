...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      login(token, user, refresh_token, false); // Pass isSignup: false
      console.log("Redirecting after login, user:", user.email, "role:", user.role);
      ...
      if (user.role === "admin") {
        ...
        // Fallback in case role is not set correctly
        navigate("/profile");
      }
    } catch (error) {
      console.error("Login failed:", error);
      let errorMessage = "Login failed. Please try again.";
      if (error.message.includes("No account found") || error.message.includes("incorrect password")) {
        errorMessage = "No account found with this email or incorrect password.";
      } else if (error.message.includes("Invalid input data") || error.message.includes("Invalid email format")) {
        errorMessage = "Please check your email and password format.";
      } else if (error.message.includes("Network error")) {
        errorMessage = "Unable to connect to the server. Please check your internet.";
          ...
          <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg
                      className="h-4 w-4 text-[#333333]/50"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
              ...
              <div className="flex items-center">
                <input type="checkbox" id="remember" className="w-3 h-3 text-[#C0399F] focus:ring-[#C0399F] rounded border-[#27AE60]" />
              ...
              >
                {isLoading ? (
                  <svg
                    className="animate-spin -ml-1 mr-2 h-3 w-3 text-[#333333]"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
