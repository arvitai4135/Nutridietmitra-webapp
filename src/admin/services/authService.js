...
const API_URL =
  import.meta.env.VITE_API_URL || "http://backend.nutridietmitra.com/api";
  ...
  try {
    ...
    if (error.response) {
      ...
      if (status === 401 || status === 404) {
        ...
        throw new Error(
          data.detail || `Server error (${status}). Please try again.`
        );
      }
...
export const fetchUserInfo = async (refreshToken) => {
  try {
    const response = await axios.get(`${API_URL}/users/info`, {
      headers: {
        Authorization: `Bearer ${refreshToken}`, // using refresh token here
        "Content-Type": "application/json",
      },
      timeout: 10000,
    });
...
axios.interceptors.request.use(
  (config) => {
    const token = sessionStorage.getItem("refresh_token"); // use refresh token
    if (token) {
